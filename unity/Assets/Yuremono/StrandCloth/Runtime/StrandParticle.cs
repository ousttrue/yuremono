...
namespace StrandCloth
{
    ...
    public class StrandParticle : MonoBehaviour
    {
        ...
        public Vector3 Collision(Vector3 newPos, IReadOnlyList<SphereCapsuleCollider> colliders)
        {
            return _Collision(newPos, _init, colliders);
        }

        public static Vector3 _Collision(Vector3 p, in ParticleInitState _init, IReadOnlyList<SphereCapsuleCollider> colliders)
        {
            foreach (var c in colliders)
            {
                if (c != null && c.TryCollide(p, _init.Radius, out var resolved))
                {
                    p += resolved.GetDelta(_init.Radius);
                }
            }
            return p;
        }
